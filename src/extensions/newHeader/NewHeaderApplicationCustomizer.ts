...
export default class NewHeaderApplicationCustomizer extends BaseApplicationCustomizer<INewHeaderApplicationCustomizerProperties> {
  ...
  private async _renderPlaceHolders(): Promise<void> {
    ...
    const selSite = this.context.pageContext.web.absoluteUrl.split("/");
    this.MainSiteName = selSite[4];
    console.log(this.MainSiteName);

    this.userPicUrl = `${this.context.pageContext.web.absoluteUrl}${
      "/_layouts/15/userphoto.aspx?size=L&accountname=" +
    ...
    const ele = React.createElement(Headercomponent, {
      user: this.userName,
      greet: this.headerGreeting,
      url: this.userPicUrl,
      uri: tenantUri,
      sitename: this.context.pageContext.web.title,
      items: this.qlItems,
      siteabsUrl: this.context.pageContext.web.absoluteUrl,
  ...
  private _onDispose(): void {
    console.log(
      "[AlertApplicationCustomizer._onDispose] Disposed custom top and bottom placeholders."
    );
  }
}
